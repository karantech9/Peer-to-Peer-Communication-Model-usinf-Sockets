...
void Sender::fileHandlingReply(Socket& si, Message& msg)
{
	...
	displayString("Peer(Client):" + att["sourcePort"] + " Sending file received acknowledgement to Peer(Server):" + att["destPort"]);
}
...
void Sender::stringHandlingRequest(Socket& si, Message& msg)
{
	...
	Verbose::show("\n", always);
	displayString("Peer(Client):" + att["sourcePort"] + " Sending one buffer of string to other Peer(Server):" + att["destPort"]);
	Verbose::show("\n", always);
	Verbose::show("Sent buffer: " + std::string(buffer), always);
}
